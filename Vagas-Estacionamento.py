...
video = cv2.VideoCapture(r'C:\Users\Pichau\Desktop\video.mp4')

while True:
  check, img = video.read()
  #Para verificar se a vaga esta livre ou não, será usada a estratégia de escala de cor dos pixels dentro de cada retângulo
  #Ou seja, vamos fazer um processo de binarização da imagem. Deixando-a em preto e branco.
  imgCinza = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
  imgTh = cv2.adaptiveThreshold(imgCinza,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,25,16)
  #Aplicando morfologia para melhorar ainda mais a imagem
  imgBlur = cv2.medianBlur(imgTh,5)
  #morfologia de dilatação de pixel.
  kernel = np.ones((3,3),np.int8)
  imgDil = cv2.dilate(imgBlur,kernel)
  qtVagasAbertas = 0
  for x,y,w,h in vagas:
    # para cada vaga, calcular a quantidade de pixels brancos
    recorte = imgDil[y:y+h,x:x+h]
    #contar a quantidade de pixels não brancos
    qtPxBranco = cv2.countNonZero(recorte)
    #para vermos o numero
